  ...
  handler: async(ctx) => {
    ...
    return await Promise.all(
      posts.map(async (post) => {
        const resolvedImageUrl = post.imageStorageId !== undefined ? await ctx
        .storage.getUrl(post.imageStorageId) : null;
...
export const searchPosts = query({
  ...
  handler: async (ctx, args) => {
    ...
    const titleMatches = await ctx.db.query('posts').withSearchIndex(
      "search_title", (q) => q.search("title", args.term)).take(limit);

      await pushDocs(titleMatches);

      if(results.length < limit) {
        const bodyMatches = await ctx.db.query('posts').withSearchIndex(
      "search_body", (q) => q.search("body", args.term)).take(limit);
